...
function adaptive_header(w,h) {
	...
	if(w<768){
		...
		$.each(navigation, function(index, val) {
			if($(this).hasClass('navigation-menu--right')){
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.navigation__column').eq(2));
				}
			}else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.navigation__column').eq(0));
				}
			}
		});
